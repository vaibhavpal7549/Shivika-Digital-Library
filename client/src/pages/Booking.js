...
export default function Booking() {
  ...
  const checkSeatAvailability = useCallback(async () => {
    try {
      const response = await apiClient.get(`/api/seats/${seatNumber}`);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      await Promise.all([checkSeatAvailability(), checkUserSeat()]);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const orderResponse = await apiClient.post("/api/create-order", {
        amount: totalFee,
        seatNumber,
        ...
        handler: async function (response) {
          ...
          try {
            ...
            if (!verifyResponse.data.bookingConfirmed) {
              ...
              throw new Error(
                "Payment successful but seat booking failed. Please contact support.",
              );
            }
            ...
            if (
              ...
              errorMessage =
                "Verification taking too long. Please check Dashboard or Contact Support.";
            }
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 max-w-md w-full">
            ...
            <p className="text-gray-600 mb-4">
              You currently have{" "}
              <strong>Seat {userExistingSeat.seatNumber}</strong> booked.
